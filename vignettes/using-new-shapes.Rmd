...
- `shapeId`: to separate different shapes

- `setEdge`, `setLabel` and `region`: each shape has three components, 'setEdge', 'setLabel' and 'region'. They are `sf` object that can be plot directly.


```{r}
plot(venn_setedge(shape1)$geometry)
```
...
ggplot(venn_setedge(shape2), aes(geometry = geometry)) +
  geom_sf()
```
...
ggplot() +
  geom_sf(aes(geometry = geometry, fill = count), data = venn_region(data)) +
  geom_sf(aes(geometry = geometry, color = id), data = venn_setedge(data), show.legend = FALSE) +
  geom_sf_text(aes(geometry = geometry, label = name), data = venn_setlabel(data)) +
  geom_sf_label(aes(geometry = geometry, label = count), data = venn_region(data)) +
  theme_void()
```
...
Then construct a `VennPlotData` class object with shapes and labels. Please note the `VennPlotData()` constructor is now moved to the new `shapeMageR` package.

```{r eval=FALSE}
shape = VennPlotData(setEdge = triangles,
                     setLabel = label_position,
                     type = "triangle",
                     shapeId = "six_triangle")
shape = unclass(shape)
```

```{r include=FALSE}
shape = get_shape_by_id("601t")
```
...
ggplot() +
  geom_sf(aes(geometry = geometry, fill = count), data = venn_region(data)) +
  geom_sf(aes(geometry = geometry, color = name), size = 2, data = venn_setedge(data)) +
  geom_sf_text(aes(geometry = geometry, label = name), data = venn_setlabel(data)) +
  theme_void() +
