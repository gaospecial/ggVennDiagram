...
ggplot() +
  ...
  geom_sf(aes(geometry = geometry, fill = id), data = venn_region(data), show.legend = FALSE) +  
  # adjust edge size and color
  geom_sf(aes(geometry = geometry), color="grey", size = 3, data = venn_setedge(data), show.legend = FALSE) +  
  # show set label in bold
  geom_sf_text(aes(geometry = geometry, label = name), fontface = "bold", data = venn_setlabel(data)) +  
  # add a alternative region name
  geom_sf_label(aes(geometry = geometry, label = name), data = venn_region(data), alpha = 0.5) +  
  theme_void()
```
...
p <- ggplot(items) +
  ...
  geom_sf_text(aes(geometry = geometry, label = name),
               ...
               inherit.aes = F) +
  geom_sf_text(aes(geometry = geometry, label = count, text = text),
               show.legend = FALSE) +
  theme_void() +
